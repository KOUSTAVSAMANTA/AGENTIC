...
def agent1(i):
    events = part_1_graph.stream(
        {"messages": ("user", i)}, config, stream_mode="values"
    )
    final_response = None

    # Process each event to find the final response
    for event in events:
        # Assume the final response is the last event from the agent
        final_response = event

    # Print or return only the final response
